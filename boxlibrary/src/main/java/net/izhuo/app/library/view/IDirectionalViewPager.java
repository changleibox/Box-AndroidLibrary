 ...
 * </p>
 */
public class IDirectionalViewPager extends ViewPager {
	...
	private DataSetObservers mObserver;
	...
	public IDirectionalViewPager(Context context) {
		super(context);
		initPager();
	}

	public IDirectionalViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		initPager();
	...
	public void setAdapter(PagerAdapter adapter) {
		...
		if (mAdapter != null) {
			if (mObserver == null) {
				mObserver = new DataSetObservers();
			}
			...
			if (mRestoredCurItem >= 0) {
				...
				setCurrentItem(mRestoredCurItem, false, true);
				mRestoredCurItem = -1;
	...
	public void setCurrentItem(int item) {
		mPopulatePending = false;
		setCurrentItem(item, true, false);
	}

	private void setCurrentItem(int item, boolean smoothScroll, boolean always) {
		...
		if (smoothScroll) {
			if (mOrientation == HORIZONTAL) {
				mSmoothScrollTo(getWidth() * item, 0);
			} else {
				mSmoothScrollTo(0, getHeight() * item);
			}
	...
	private void mAddNewItem(int position, int index) {
	...
	private void mDataSetChanged() {
		...
		if (newCurrItem >= 0) {
			setCurrentItem(newCurrItem, false, true);
			needPopulate = true;
		}
		if (needPopulate) {
			mPopulate();
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (mAdapter != null) {
			...
			setCurrentItem(ss.position, false, true);
		} else {
	...
	private ItemInfo mInfoForChild(View child) {
		for (int i = 0; i < mItems.size(); i++) {
			ItemInfo ii = mItems.get(i);
			...
			if (mIsBeingDragged) {
				...
				if ((Math.abs(initialVelocity) > mMinimumVelocity)
					...
					if (lastMotion > mInitialMotion) {
						setCurrentItem(mCurItem - 1, true, true);
					} else {
						setCurrentItem(mCurItem + 1, true, true);
					}
				} else {
					setCurrentItem(mCurItem, true, true);
				}
    ...
    private class DataSetObservers extends DataSetObserver {
        @Override
