 ...
 *         2015年3月28日
 */
public class ISocketHttpRequester {
	/**
	...
	public static boolean post(String path, Map<String, String> params,
			IFormFile[] files) throws Exception {
		final String BOUNDARY = "---------------------------7da2137580612"; // 数据分隔线
		final String endline = "--" + BOUNDARY + "--\r\n";// 数据结束标志

		int fileDataLength = 0;
		for (IFormFile uploadFile : files) {// 得到文件类型数据的总长度
		...
		// 把所有文件类型的实体数据发送出来
		for (IFormFile uploadFile : files) {
	 ...
	 */
	public static boolean post(String path, Map<String, String> params,
			IFormFile file) throws Exception {
		return post(path, params, new IFormFile[] { file });
	}
}
